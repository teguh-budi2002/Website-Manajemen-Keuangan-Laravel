...
<div class="w-full flex justify-center mt-28">
    <div class="w-2/3 bg-white rounded p-2">
        ...
        <form action="{{ URL("cash-out/send/" . $balance->id) }}" method="POST">
            @csrf
            @method("PUT")
            <div class="sub_header_form mt-5">
                ...
                <div class="bg-gray-200 mt-2 w-fit p-2 rounded border">
                    <p class="text-xl font-semibold text-gray-600">
                        {{ $balances !== NULL ? number_format($balance->balance, 2) : "0" }} <span>IDR</span></p>
                </div>
            </div>
            <div class="form-group mt-5 flex items-center space-x-6">
                ...
                <div class="w-full">
                    <select name="category_id"
                        ...
                        <option value="{{ $balance->category->id }}" class=" text-gray-500 uppercase">-
                            {{ $balance->category->name_category }}</option>
                        {{-- @if (isset($balances->category))
                        @endif --}}
                    </select>
                </div>
            </div>
            <div class="form_group mt-3">
                ...
                <textarea id="keterangan" rows="4" name="description"
                    ...
                    placeholder="Masukkan Keterangan Anda">{{  $balance->description  }}</textarea>
            </div>
